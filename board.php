	...
	if ($board_class->board_locked == 0 || ($user_authority > 0 && $user_authority != 3)) {
		...
		if ((file_exists(TC_BOARDSDIR . $board_class->board_dir . '/src/' . $upload_class->file_name . $upload_class->file_type) && file_exists(TC_BOARDSDIR . $board_class->board_dir . '/thumb/' . $upload_class->file_name . 's' . $upload_class->file_type)) || ($file_is_special && file_exists(TC_BOARDSDIR . $board_class->board_dir . '/src/' . $upload_class->file_name . $upload_class->file_type)) || $post_fileused == false) {
			...
			if ($is_oekaki) {
				if (file_exists(TC_BOARDSDIR . $board_class->board_dir . '/src/' . $upload_class->file_name . '.pch')) {
					$post['message'] .= '<br><small><a href="' . TC_CGIPATH . '/animation.php?board=' . $board_class->board_dir . '&id=' . $upload_class->file_name . '">View animation</a></small>';
				}
			}
			
			if ($thread_replyto != '0') {
				if ($post['message'] == '' && TC_NOMESSAGEREPLY != '') {
					$post['message'] = TC_NOMESSAGEREPLY;
				}
			} else {
				if ($post['message'] == '' && TC_NOMESSAGETHREAD != '') {
					$post['message'] = TC_NOMESSAGETHREAD;
				}
			}
			...
			if ($user_authority > 0 && $user_authority != 3) {
				...
				management_addlogentry($modpost_message, 1, md5_decrypt($_POST['modpassword'], TC_RANDOMSEED));
			}
			
			if ($post['name_save']) {
				setcookie('name', urldecode($_POST['name']), time() + 31556926, '/', TC_DOMAIN);
			}
			
			if ($post['email_save']) {
				setcookie('email', urldecode($post['email']), time() + 31556926, '/', TC_DOMAIN);
			}
		...
		if (TC_WATCHTHREADS && $thread_replyto != '0') {
			...
			if ($viewing_thread_is_watched > 0) {
				$newestreplyid = $tc_db->GetOne('SELECT `id` FROM `'.TC_DBPREFIX.'posts_'.$board_class->board_dir.'` WHERE `IS_DELETED` = 0 AND `parentid` = '.$thread_replyto.' ORDER BY `id` DESC LIMIT 1');
		...
		if (TC_INSTANTREDIRECT && ($board_class->board_redirecttothread == 1 || $_POST['em'] == 'return' || $_POST['em'] == 'noko')) {
			if ($thread_replyto == '0') {
				...
				header('Location: ' . TC_BOARDSPATH . '/' . $board_class->board_dir . '/res/' . $post_id . '.html');
			} else {
				/* Regenerate the thread */
				$board_class->RegenerateThread($thread_replyto);
				header('Location: ' . TC_BOARDSPATH . '/' . $board_class->board_dir . '/res/' . $thread_replyto . '.html');
			}
			...
			if (md5($_POST['postpassword']) == $post_class->post_password) {
				if (isset($_POST['fileonly'])) {
					...
					if ($post_class->Delete()) {
						...
						$board_class->RegeneratePages();
						die('regenerating: ' . $post_class->post_parentid);
