...
function config_getvalue($key) {
    global $tc_db;
    $query = "SELECT `value` FROM `".TC_DBPREFIX."config` WHERE `key` = '$key'";
...
function config_setvalue($key,$value) {
    global $tc_db;
    $results = $tc_db->SelectLimit("SELECT * FROM `".TC_DBPREFIX."config` WHERE `key` = '".$key."'",1);
    if (count($results)>0) {
        $tc_db->Execute("UPDATE `".TC_DBPREFIX."config` SET `value` = '".$value."' WHERE `key` = '".$key."'");
        return true;
    } else {
        return false;
    }
...
function print_stylesheets($prefered_stylesheet = 'Burichan') {
    ...
    if ($prefered_stylesheet=='Burichan') {
        return '<link rel="stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/burichan.css" title="Burichan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/futaba.css" title="Futaba">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/gurochan.css" title="Gurochan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/photon.css" title="Photon">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/fuhrerchan.css" title="Fuhrerchan">';
    } else if ($prefered_stylesheet=='Futaba') {
        return '<link rel="stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/futaba.css" title="Futaba">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/burichan.css" title="Burichan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/gurochan.css" title="Gurochan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/photon.css" title="Photon">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/fuhrerchan.css" title="Fuhrerchan">';
    } else if ($prefered_stylesheet=='Gurochan') {
        return '<link rel="stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/gurochan.css" title="Gurochan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/burichan.css" title="Burichan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/futaba.css" title="Futaba">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/photon.css" title="Photon">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/fuhrerchan.css" title="Fuhrerchan">';
    } else if ($prefered_stylesheet=='Photon') {
        return '<link rel="stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/photon.css" title="Photon">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/burichan.css" title="Burichan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/futaba.css" title="Futaba">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/gurochan.css" title="Gurochan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/fuhrerchan.css" title="Fuhrerchan">';
    } else if ($prefered_stylesheet=='Fuhrerchan') {
        return '<link rel="stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/fuhrerchan.css" title="Fuhrerchan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/burichan.css" title="Burichan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/futaba.css" title="Futaba">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/gurochan.css" title="Gurochan">
    <link rel="alternate stylesheet" type="text/css" href="'.TC_BOARDSPATH.'/css/photon.css" title="Photon">';
...
function check_md5($md5,$board) { //Checks if the supplied md5 file hash is currently recorded inside of the database, attached to a non-deleted post.
    global $tc_db;
    $results = $tc_db->GetAssoc("SELECT `id`,`threadid` FROM `".TC_DBPREFIX."posts_".mysql_real_escape_string($board)."` WHERE `imagemd5` = '".mysql_real_escape_string($md5)."' AND `IS_DELETED` = 0 LIMIT 1");
    if (!empty($results)) {
        foreach($results AS $line) {
...
function format_postboxnotice($notice,$board) {
    ...
    foreach($results AS $line) {
        ...
        $replace = array (config_getvalue('maxthumbwidth'),config_getvalue('maxthumbheight'),display_numuniqueposts($board),round($line['maximagesize']/1024),$filetypes);
...
function boardid_to_dir($boardid) {
    global $tc_db;
    $results = $tc_db->CacheSelectLimit("SELECT `name` FROM `".TC_DBPREFIX."boards` WHERE `id` = '".mysql_real_escape_string($boardid)."'",1);
    if (count($results)>0) {
        foreach($results AS $line) {
            return $line['name'];
        }
...
function flushthreadrepliescache($threadid,$board) {
    ...
    if (TC_DBUSECACHE) {
        ...
        $tc_db->CacheFlush('SELECT * FROM `'.TC_DBPREFIX.'posts_'.mysql_real_escape_string($board).'` WHERE `IS_DELETED` = 0 AND `threadid` = '.mysql_real_escape_string($threadid).' ORDER BY `postedat` DESC LIMIT '.config_getvalue('numrepliesdisplayed'));
        $tc_db->CacheFlush('SELECT * FROM `'.TC_DBPREFIX.'posts_'.mysql_real_escape_string($board).'` WHERE `IS_DELETED` = 0 AND `threadid` = '.mysql_real_escape_string($threadid).' ORDER BY `postedat` DESC LIMIT '.config_getvalue('numrepliesdisplayedsticky'));
    }
}
