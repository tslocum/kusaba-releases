...
function delandbanlinks(spanid,board,postid,isthread) {
	...
	var newhtml = '';
	newhtml += '&nbsp;&#91;<a href="' + tc_cgipath + '/manage_page.php?action=delposts&boarddir=' + board + '&del';
	if (isthread) {
		newhtml += 'thread';
	} else {
		newhtml += 'post';
	}
	newhtml += 'id=' + postid + '" title="Delete" onclick="return confirm(\'Are you sure you want to delete this post/thread?\');">D<\/a>&nbsp;<a href="' + tc_cgipath + '/manage_page.php?action=delposts&boarddir=' + board + '&del';
	if (isthread) {
		newhtml +='thread';
	} else {
		newhtml += 'post';
	}
	newhtml +='id=' + postid + '&postid=' + postid + '" title="Delete &amp; Ban" onclick="return confirm(\'Are you sure you want to delete and ban the poster of this post/thread?\');">&amp;<\/a>&nbsp;<a href="' + tc_cgipath + '/manage_page.php?action=bans&banboard=' + board + '&banpost=' + postid + '" title="Ban">B<\/a>&#93;';
...
function expandthread(threadid, board) {
	if (document.getElementById('replies' + threadid + board)) {
		...
		new Ajax.Request(tc_boardspath + '/expand.php?board=' + board + '&threadid=' + threadid,
		{
			method:'get',
...
function getwatchedthreads(threadid, board) {
	if (document.getElementById('watchedthreadlist')) {
		...
		new Ajax.Request(tc_boardspath + '/threadwatch.php?board=' + board + '&threadid=' + threadid,
		{
			method:'get',
			onSuccess: function(transport){
				var response = transport.responseText || "something went wrong (blank response)";
...
function addtowatchedthreads(threadid, board) {
	if (document.getElementById('watchedthreadlist')) {
		new Ajax.Request(tc_boardspath + '/threadwatch.php?do=addthread&board=' + board + '&threadid=' + threadid,
		{
			method:'get',
			onSuccess: function(transport){
				var response = transport.responseText || "something went wrong (blank response)";
...
function removefromwatchedthreads(threadid, board) {
	if (document.getElementById('watchedthreadlist')) {
		new Ajax.Request(tc_boardspath + '/threadwatch.php?do=removethread&board=' + board + '&threadid=' + threadid,
		{
			method:'get',
			onSuccess: function(transport){
				var response = transport.responseText || "something went wrong (blank response)";
...
function expandimg(postnum, imgurl, thumburl, imgw, imgh, thumbw, thumbh) {
	...
	if (element.innerHTML != thumbhtml) {
...
function set_delpass(id) {
	if (document.getElementById(id).postpassword) {
		with(document.getElementById(id)) {
			if(!postpassword.value) postpassword.value=getCookie("postpassword");
		}
	}
}
